...
function renderStars(value) {
  return (
    <span className="flex gap-0.5" title={`${value} estrela${value === 1 ? '' : 's'}`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <svg
          key={i}
      ...
      <div className="flex flex-col gap-1 mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium flex items-center gap-1">
            De:
            <span className="truncate max-w-[180px]" title={autor?.nome || (feedback.anonimo ? "Anônimo" : "Desconhecido")}>{autor?.nome || (feedback.anonimo ? "Anônimo" : "Desconhecido")}</span>
            {autor?.cargo && <>
              <span className="text-gray-400"> • </span>
              <span className="truncate max-w-[180px] text-gray-500" title={autor.cargo}>{autor.cargo}</span>
            </>}
          </span>
        </div>
        {showDestinatario && destinatario && (
            ...
            <span className="text-sm font-medium flex items-center gap-1">
              Para:
              <span className="truncate max-w-[180px]" title={destinatario.nome}>{destinatario.nome}</span>
              {destinatario.cargo && <>
                <span className="text-gray-400"> • </span>
                <span className="truncate max-w-[180px] text-gray-500" title={destinatario.cargo}>{destinatario.cargo}</span>
              </>}
            </span>
          </div>
        )}
